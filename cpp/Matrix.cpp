...
{
 	if (ones)
        this->data = std::vector<std::vector<double>>(m, std::vector<double>(n, 1));
 	else
...
Matrix::Matrix(std::optional<Matrix>a)
{	
	this->data=a->data;
...
Matrix Matrix::inv()
{
	...
	{
		std::cout << "The Matrix det is zero and doesnt has a inverse" << std::endl;
		std::vector<std::vector<double>> M(size[0], std::vector<double>(size[1], 0));
		Matrix zeros{ M };
		return zeros;
	}
